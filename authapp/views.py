...
from authapp.models import Contact,MembershipPlan,Trainer,Enrollment,Gallery,Attendance,about,Team,Service,Home_head,Home_welcome
...
def Home(request):
    heads=Home_head.objects.all()
    welcome=Home_welcome.objects.all()
    context={'heads':heads,'welcome':welcome}
    return render(request, "index.html",context)

def About(request):
    posts=about.objects.all()
    teams=Team.objects.all()
    context={'posts':posts,'teams':teams}
...
def attendance(request):
    ...
    if request.method=="POST":
        ...
        querydata=Attendance(phonenumber=phonenumber,Login=Login,Logout=Logout,SelectWorkout=SelectWorkout,TrainedBy=TrainedBy)
        querydata.save()
...
def signup(request):
    if request.method=="POST":
        ...
        if len(username)>12 or len(username)<12:
            messages.info(request,"Phone Number Must be 12 Digits")
